/**
 * This code comes from: https://github.com/antfu/markdown-it-github-alerts?tab=readme-ov-file
 * 
 * I modified it to allow using the `aside` html element, as it is the appropriate tag to use.
 */
...
const MarkdownItGitHubAlerts: MarkdownIt.PluginWithOptions<MarkdownItGitHubAlertsOptions> = (md, options = {}) => {
	...
	md.renderer.rules.alert_open = function (tokens, idx) {
		...
		return `<aside class="${classPrefix} ${classPrefix}-${type}"><p class="${classPrefix}-title">${icon}<span>${title}</span></p>`
	}
}
